...
void oops(char const* param) {
  ...
  // If you still pass a it like we did in program3, it is going to get copied
  // in internal storage of thread like this and then will be passed as rvalue
  // to f() But now f() wants a non-const ref. and so you are going to get error
